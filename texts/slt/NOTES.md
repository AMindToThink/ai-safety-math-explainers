...
| DSLT 1 §"The RLCT can be read off when K(w) is in normal crossing form" (one- and multi-dim cases) | `dslt1-normal-crossing-game` | Polynomial K in normal crossing form, 1D/2D | Game: given K(w) = ∏ w_i^(2k_i) (possibly with shifted critical points, à la Example 1.5), read off the local and global RLCT and multiplicity | in progress | |
...
- **Chapters complete:** 0
- **Currently being drafted:** Five DSLT 1 widgets shipped as
  ...
  `dslt1-bic-derivation` (module 4), and
  `dslt1-volume-scaling-rlct` (module 5). Three remaining DSLT 1
